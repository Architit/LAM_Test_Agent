...
2. **EVIDENCE-FIRST ANALYZE:** Сначала собрать evidence: `git status --short`, `git diff --stat`, целевой лог (`cat logs/error.log` или эквивалент).
3. **SAFE CONTAINMENT:** Перевести поток в `HOLD_BY_DEADLOOP_BREAK_PROTOCOL` и зафиксировать причину в `DEV_LOGS.md`.
4. **NON-DESTRUCTIVE RECOVERY:** Использовать только точечные/обратимые действия (single-file restore, feature-branch fix, targeted revert commit).
5. **DESTRUCTIVE ROLLBACK (LAST RESORT):** Глобальные команды (`git restore .`, `git checkout main`, `reset --hard`) допускаются только после явного подтверждения оператора и с зафиксированным evidence.
6. **PLAN:** Предложить *один* следующий диагностический шаг за итерацию.
...
## 10. Ecosystem Protocol Expansion (Safety/Resource Stack)

Для межрепозиторного развития экосистемы (agent/core/provider/gateway) вводится обязательный многослойный safety/resource стек.

Источники cross-repo derivation:
- `.gitmodules`
- `TEST_MIRROR_MATRIX.md`
- `COMPATIBILITY_MATRIX_REPORT_2026-02-17.md`
- `GATEWAY_ACCESS_CONTRACT.md`

Канонический machine-readable стек:
- `memory/FRONT/ECOSYSTEM_SAFETY_RESOURCE_STACK_V1.json`

Обязательные правила:
1. `change_budget` считается только одним слоем и не может использоваться как единственный механизм защиты.
2. Проверка safety/resource stack обязательна в quality-gate CI.
3. Автогенерация growth/backlog допускается только в bounded-режиме (hard caps, deterministic order, no recursion).
4. При провале provisioning layers (`network_resolution_gate`, `submodule_readiness_gate`) live-режим блокируется и используется безопасный fallback.

---

## 11. Synaptic Learning Signals (Phase 12.0)

Для задач, выполняемых в контуре LRPT, вводится обязательная precondition-проверка статической памяти опыта:

- `LRPT/protocol/LEARNING_SIGNAL_CONTRACT_V1.md`
- `LRPT/journal/SYNAPTIC_WEIGHTS_V1.yaml`
- `LRPT/flow/TASK_SPEC_SYNAPTIC_PLASTICITY_V1.yaml`

Обязательные правила:
1. Перед Action Block агент MUST читать актуальные веса из `SYNAPTIC_WEIGHTS_V1.yaml`.
2. При срабатывании паттерна зацикливания (`w_anti_samsara_001`) агент MUST остановить генерацию и перевести поток в `HOLD_BY_DEADLOOP_BREAK_PROTOCOL`.
3. Изменения learning-signal контрактов/весов допускаются только как governance-изменение с semantic tag.

---

## 12. External Debug Routing for Codex/OpenAI Code-Fix Requests

Для запросов исправления кода, где используется внешний контур Codex/OpenAI, применяется расширенный debug routing:

1. Runtime debug stream формируется в JSONL (см. `lam_logging.py`) и может зеркалироваться во внешний debug-файл.
2. Ручная выгрузка debug-лога во внешний gateway:
   - `scripts/gateway_io.sh export-debug [log_file]`
3. Ручная отправка debug-лога в endpoint OpenAI/внешней системы:
   - `scripts/gateway_io.sh send-openai-debug [log_file]`
4. Подтверждение доставки (`delivery evidence`) считается валидным только при двух условиях одновременно:
   - HTTP status `2xx`
   - создан receipt-файл в `OPENAI_DEBUG_RECEIPTS_DIR`
5. Без receipt-файла статус отправки считается `UNCONFIRMED`, даже если оператор предполагает успешную отправку.
6. Настоящий протокол фиксирует только транспорт и traceability; влияние на улучшение моделей/агента OpenAI не может гарантироваться локально.
