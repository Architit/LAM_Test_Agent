...
def test_sensitive_put_requires_contract_and_approval(tmp_path, monkeypatch) -> None:
    module = load_gateway_module()
    state_dir = tmp_path / ".gateway"
    monkeypatch.setenv("LAM_GATEWAY_STATE_DIR", str(state_dir))
    monkeypatch.setenv("LAM_GATEWAY_POLICY_FILE", str(state_dir / "routing_policy.json"))
    ...
    try:
        ...
        assert "contract_id required" in str(exc)
    else:
        raise AssertionError("expected contract gate failure")


def test_restricted_class_blocks_external_provider(tmp_path, monkeypatch) -> None:
    module = load_gateway_module()
    state_dir = tmp_path / ".gateway"
    gdrive_root = tmp_path / "gdrive"
    monkeypatch.setenv("LAM_GATEWAY_STATE_DIR", str(state_dir))
    ...
    source.write_text("restricted", encoding="utf-8")
    ...
    try:
        ...
        assert "not allowed for class 'restricted'" in str(exc)
    else:
        raise AssertionError("expected provider allowlist failure")


def test_circulation_kill_switch_blocks_put(tmp_path, monkeypatch) -> None:
    module = load_gateway_module()
    state_dir = tmp_path / ".gateway"
    monkeypatch.setenv("LAM_GATEWAY_STATE_DIR", str(state_dir))
    monkeypatch.setenv("LAM_GATEWAY_POLICY_FILE", str(state_dir / "routing_policy.json"))
    ...
    assert module.cmd_circulation_kill_switch(KillArgs()) == 0
    ...
    try:
        ...
        raise AssertionError("expected kill-switch denial")

    class KillOffArgs:
        action = "off"

    assert module.cmd_circulation_kill_switch(KillOffArgs()) == 0
